...
const roomFields = `{
  _id, name, "slug": slug.current, type, description, heroImage, gallery,
  rate, sqm, occupancy, floor, view, amenities, featured, active
}`;

const experienceFields = `{
  _id, name, "slug": slug.current, category, description, heroImage,
...
const journalFields = `{
  ...
  readingTime, excerpt, heroImage, featured
}`;
...
export const JOURNAL_BY_SLUG_QUERY = `*[_type == "journalPost" && slug.current == $slug][0] {
  _id, title, "slug": slug.current, category, author, publishedAt,
  readingTime, excerpt, body, heroImage, featured
}`;
